...
- **Attach the card by uploading it, not by URL.** The post should carry the dashboard OG card
  as its image (see **Image attachment** below), but the cloud flow sends Telegram *before* the
  commit is pushed, so the card served by the site is still the previous dataset's. Render this
  run's card locally and upload it with the request instead — `scripts/og/` is tracked, so it's
  in the checkout:
  ```bash
  pnpm og      # regenerates public/og/*.png from the new default dataset
  curl -s -X POST "https://fragmata.info/api/internal/telegram" \
    -H "Authorization: Bearer $INTERNAL_API_SECRET" \
    -F "text=<$post.txt" \
    -F "photo=@public/og/dashboard.en.png;type=image/png"
  ```
  Run `pnpm og` **after** `dataManager.ts` points at the new dataset, or you'll upload the old
  card. Pass the body via `-F "text=<file"` rather than inline, to avoid shell-quoting problems;
  add `-F "dryRun=true"` to preview (the response echoes `photo: "upload:… (N bytes)"` so you can
  confirm the file arrived). Uploads must be PNG or JPEG and under 4MB — a card is ~120KB.
  Add `-F "noPhoto=true"` for a text-only post if the card can't be rendered; a post with no
  image beats a post whose image contradicts it.
  The JSON body form (above) attaches the *deployed* card by URL — correct for local runs that
  post after the deploy, wrong here.
...
- Use `--dry-run` to print the length, the photo URL, and a preview without sending
- The script prints the returned `message_id` and `chat_id` on stdout — capture them

**Image attachment** — every post goes out as a photo with the text as its caption, so
forwarding the single message carries the whole package:

- The image is the dashboard OG card. A local run attaches it by URL —
  `https://fragmata.info/og/dashboard.en.png?v=<report date>`, built by the script itself from
  the current default dataset, so you don't pass it. The `?v=` is the cache-buster that makes
  Telegram (and the CDN) fetch *this* update's card rather than yesterday's.
- **A URL-attached card only works after the push to `main` has deployed.** The card is a
  static file regenerated by `pnpm og` during the build; posting before the deploy attaches the
  previous dataset's numbers. Cloud runs post before the push, so they upload a locally
  rendered card instead — see **Cloud runs** above.
- Posts up to 1024 chars (Telegram's caption cap) go as one photo+caption message — that covers
  nearly every post. A longer one degrades gracefully: card + title line first, then the full
  text as a second message. Record the first message's `message_id` in `community/TELEGRAM.md`
  as usual.
- Overrides, rarely needed: `--photo <url>` for a different card (e.g. a dam card when the post
  is about one dam), `--no-photo` for text-only.
