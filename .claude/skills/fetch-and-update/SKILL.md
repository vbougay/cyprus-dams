...
- **Telegram** replaces `tsx scripts/post-telegram.ts`:
  ...
  curl -s -X POST "https://fragmata.info/api/internal/telegram" \
    ...
    -d @post.json   # {"text": "…", "noPhoto": true}   add "dryRun": true to preview
  ```
...
- **Cloud runs post text-only — always pass `"noPhoto": true`.** Local runs attach the
  dashboard OG card (see **Image attachment** below), but that card is a static PNG rebuilt
  by the Vercel deploy, and the cloud flow sends Telegram *before* the commit is pushed. The
  attached card would therefore carry the *previous* dataset's numbers while the text carries
  the new ones. Until the relay can take a freshly rendered card, omit the image here rather
  than ship a contradiction.
...
**Image attachment** — a local post goes out as a photo with the text as its caption, so
forwarding the single message carries the whole package:

- The image is the site's dashboard OG card, `https://fragmata.info/og/dashboard.en.png?v=<report date>`.
  The script builds this URL itself from the current default dataset — you don't pass it. The
  `?v=` is the cache-buster that makes Telegram (and the CDN) fetch *this* update's card
  rather than yesterday's.
- **Send only after the push to `main` has deployed.** The card is a static file regenerated by
  `pnpm og` during the build; posting before the deploy attaches the previous dataset's numbers.
  (This is exactly why cloud runs go text-only — see **Cloud runs** above.)
